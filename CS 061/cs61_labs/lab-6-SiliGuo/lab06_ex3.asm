...
;		x can be 0 or 1; if user input an invalid input, the system
;		will output an error message and let user to re-enter that
;		digit.
;Return Value: R2
;---------------------------------------------------------------------------
.ORIG x3400
...
CHECK_LOOP
	...
	BRz END_CHECK_LOOP		; if (R4 == 0) END_CHECK_LOOP

	LEA R0, ERROR_MSG
	PUTS				; Ouput error message
	BRnzp CHECK_LOOP		; Loop
END_CHECK_LOOP
...
INPUT_LOOP
	...
	ADD R4, R4, #-1			; R4 <-- R4 - #1
	BRz ADD_ONE			; if (R4 == 0) ADD_ONE
	AND R4, R4, #0			; R4 <-- #0
	LD R3, N_SPACE			; R3 <--#-32
	ADD R4, R0, R3			; R4 <-- R0 + R3
	BRz INPUT_LOOP			; if (R4 == 0) INPUT_LOOP
	BRnp ERROR_2			; if (R4 != 0) ERROR_2
	...
	AND R4, R4, #0			; R4 <-- #0
	ADD R4, R1, #-1			; R4 <-- R1 - #1
	BRz END_INPUT_LOOP		; if (R4 == 0) INPUT_LOOP
...
N_ASC_b		.FILL	#-98		; #-98 stores here
N_SPACE		.FILL	#-32		; #-32 stores here
ERROR_MSG	.STRINGZ	"\nInvalid input! Please enter letter 'b' before 16-bit binary number:\n"
ERROR_INPUT	.STRINGZ	"\nInvalid input! Please only enter 0 or 1 for the 16-bit binary number:\n"
